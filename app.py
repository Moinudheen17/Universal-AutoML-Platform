...
if uploaded_file is None:
    ...
    with st.expander("🔍 Exploratory Data Analysis"):
        ...
        with col1:
            ...
            ax_dist.tick_params(colors='rgba(255,255,255,0.4)')
            ax_dist.spines[:].set_color('rgba(255,255,255,0.06)')
            st.pyplot(fig_dist)
        with col2:
            ...
            if not numeric_df.empty and len(numeric_df.columns) > 1:
                ...
                ax_corr.tick_params(colors='rgba(255,255,255,0.4)')
                st.pyplot(fig_corr)
            else:
    ...
    if os.path.exists(report_path) and os.path.exists(model_path):
        ...
        with tab1:
            ...
            if metric in report.columns:
                ...
                ax_bar.set_title(f"Model Comparison — {metric}", color='rgba(255,255,255,0.5)', fontsize=11)
                ax_bar.tick_params(colors='rgba(255,255,255,0.4)')
                ax_bar.spines[:].set_color('rgba(255,255,255,0.05)')
                ax_bar.set_xlabel(metric, color='rgba(255,255,255,0.3)', fontsize=10)
                ax_bar.set_ylabel("")
                st.pyplot(fig_bar)
        ...
        with tab2:
            ...
            try:
                ...
                if is_classification:
                    ...
                    ax_eval.set_title("Confusion Matrix", color='rgba(255,255,255,0.5)', fontsize=12)
                    ax_eval.set_xlabel("Predicted", color='rgba(255,255,255,0.3)', fontsize=11)
                    ax_eval.set_ylabel("Actual", color='rgba(255,255,255,0.3)', fontsize=11)
                else:
                    ax_eval.scatter(y_eval, y_pred_eval, alpha=0.55, color='#00e5c2', edgecolors='rgba(0,0,0,0.3)', s=40)
                    mn = min(y_eval.min(), y_pred_eval.min())
                    mx = max(y_eval.max(), y_pred_eval.max())
                    ax_eval.plot([mn, mx], [mn, mx], '--', color='rgba(255,255,255,0.2)', lw=1.5, label="Perfect")
                    ax_eval.set_title("Actual vs Predicted", color='rgba(255,255,255,0.5)', fontsize=12)
                    ax_eval.set_xlabel("Actual", color='rgba(255,255,255,0.3)')
                    ax_eval.set_ylabel("Predicted", color='rgba(255,255,255,0.3)')
                    ax_eval.legend(labelcolor='rgba(255,255,255,0.4)', framealpha=0)
                ax_eval.tick_params(colors='rgba(255,255,255,0.35)')
                ax_eval.spines[:].set_color('rgba(255,255,255,0.05)')
                st.pyplot(fig_eval)
            except Exception as e:
            ...
            try:
                ...
                if importances is not None:
                    ...
                    ax_imp.set_title("Top Features", color='rgba(255,255,255,0.5)', fontsize=11)
                    ax_imp.tick_params(colors='rgba(255,255,255,0.35)')
                    ax_imp.spines[:].set_color('rgba(255,255,255,0.05)')
                    ax_imp.set_xlabel("Importance", color='rgba(255,255,255,0.3)')
                    ax_imp.set_ylabel("")
                    st.pyplot(fig_imp)
                else:
